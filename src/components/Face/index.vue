...
const facebg = require('../../assets/images/face.png');
export default {
  name: "H5FaceCropper",
  ...
  created() {
    ...
    this.initImportOpenCvModel()
  },
  mounted() {
    this.$refs.cropper.cropW = this.option.autoCropHeight
    this.$refs.cropper.cropH = this.option.autoCropWidth
  },
  methods: {
    //取消裁剪
    canceltailor(){
    ...
    async tailoring() {
      ...
      this.$refs.cropper.getCropData(async (data) => {
        ...
        const loadImg = await this.saveCanvas("canvesImg", data);
        if (loadImg) {
          const face = await this.checkFaceNum();
    ...
    addsolide() {
      if (document.getElementById("vertical") == null) {
        ...
        // 背景头像图
        let verticalbg = document.createElement("div");
        verticalbg.style.width = this.option.autoCropWidth + "px";
        verticalbg.style.height = this.option.autoCropHeight + "px";
        verticalbg.style.backgroundImage = `url(${facebg})`;
        verticalbg.style.backgroundSize = "100% 100%";
        verticalbg.style.position = "absolute";
        verticalbg.style.top = "0px";
        verticalbg.style.left = "0px";
        verticalbg.style.pointerEvents = "none";
    ...
    // 将截取的数据存入canvas中，用于后续人像识别使用
    async saveCanvas(canvasId, data) {
      return new Promise((resolve) => {
